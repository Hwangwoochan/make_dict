...
class Lemmatizer:
    ...
    def check_word_exists(self, word, file_path):
        with open(file_path, "r", encoding="utf-8") as file:
            words = set(line.strip() for line in file)
        
        for i in range(1, len(word) + 1):  # 앞에서부터 1글자씩 증가하며 확인
            base_word = word[:i]
            if base_word in words:
                variations = ((base_word + "하", "Verb"),
                            (base_word + "해주", "Verb"),
                            (base_word + "해보", "Verb"))
                return [variations]
    ...
    def analyze(self, word, debug=False):
        ...
        base_path = os.path.dirname(os.path.abspath(__file__))  # 현재 실행되는 스크립트의 절대 경로
        file_path = os.path.join(base_path, "dictionary/default/Acting_Noun.txt")

       
        pre = self.check_word_exists(word,file_path)
    
        if pre:  # pre가 None, 빈 리스트, 빈 튜플, 빈 문자열이 아니면 반환
            return pre
        else:
